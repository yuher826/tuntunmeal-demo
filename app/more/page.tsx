      ...
      <div style={{ width: '100%', maxWidth: 390, background: '#F0F4EC', fontFamily: 'sans-serif' }}>
        ...
        <div style={{ margin: '16px', background: '#fff', borderRadius: 12, overflow: 'hidden' }}>
          ...
          {[
            ...
            { icon: '🎁', label: '이벤트' },
            { icon: '⭐', label: '앱 평가하기' },
            { icon: '📤', label: '친구에게 공유하기' },
          ].map((item, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', borderTop: i === 0 ? 'none' : '1px solid #f0f0f0', cursor: 'pointer' }}>
        ...
        <div style={{ margin: '0 16px 32px' }}>
          <button onClick={() => setShowLogout(true)} style={{ width: '100%', padding: '16px', borderRadius: 12, border: '1px solid #ddd', background: '#fff', color: '#C0392B', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
            로그아웃
          </button>
        </div>
