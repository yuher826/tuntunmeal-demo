...
export default function Home() {
    ...
    const fetchWeather = async () => {
      try {
        ...
        if (cached) {
          ...
          if (Date.now() - timestamp < CACHE_DURATION) { setWeather(data); return; }
        }
        ...
        const data = { temp: Math.round(json.main.temp), desc: json.weather[0].description, icon: json.weather[0].main };
        localStorage.setItem(WEATHER_CACHE_KEY, JSON.stringify({ data, timestamp: Date.now() }));
        setWeather(data);
      } catch (e) { console.log('날씨 불러오기 실패', e); }
    };
    fetchWeather();
  }, []);

  const getWeatherEmoji = (icon: string) => {
    const map: Record<string, string> = { Clear: '☀️', Clouds: '☁️', Rain: '🌧️', Drizzle: '🌦️', Snow: '❄️', Thunderstorm: '⛈️', Mist: '🌫️', Fog: '🌫️' };
    return map[icon] || '🌤️';
  };
  ...
  const maxKcal = Math.max(...menus.map(m => m.kcal));
    ...
    <div style={{ minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ background: '#2C3E1A', padding: '16px 16px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ background: '#3B6D11', borderRadius: 14, padding: '4px 12px', fontSize: 10, fontWeight: 700, color: '#fff' }}>AM 5:00 부터</div>
        </div>
      </div>
      ...
      <div style={{ padding: '14px 14px 8px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={() => setShowCalorie(!showCalorie)}
              style={{ background: showCalorie ? '#3B6D11' : '#EAF3DE', color: showCalorie ? '#fff' : '#3B6D11', border: 'none', borderRadius: 10, padding: '3px 10px', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>
              🔥 칼로리 비교
            </button>
            {/* ✅ 2가지 선택 버튼 - 이제 클릭 가능! */}
            <button onClick={() => setShowCompare(true)}
              style={{ background: showCompare ? '#3B6D11' : '#EAF3DE', color: showCompare ? '#fff' : '#3B6D11', border: 'none', borderRadius: 10, padding: '3px 10px', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>
              ⚖️ 2가지 선택
            </button>
          </div>
        </div>
        ...
        {showCalorie && (
          <div style={{ background: '#fff', borderRadius: 12, padding: '14px', marginBottom: 12 }}>
            <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 10, color: '#333' }}>🔥 칼로리 비교</div>
            {menus.map(menu => (
              <div key={menu.id} style={{ marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4, fontSize: 12 }}>
                  <span style={{ fontWeight: 600 }}>{menu.emoji} {menu.name}</span>
                  <span style={{ color: menu.kcal < 450 ? '#3B6D11' : '#e67e22', fontWeight: 700 }}>{menu.kcal}kcal</span>
                </div>
                <div style={{ height: 10, background: '#f0f0f0', borderRadius: 5 }}>
                  <div style={{ width: `${(menu.kcal / maxKcal) * 100}%`, height: '100%', background: menu.kcal < 450 ? '#3B6D11' : '#e67e22', borderRadius: 5, transition: 'width 0.5s' }} />
                </div>
                <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
                  {[
                    { label: '탄수화물', value: menu.nutrition.carb, color: '#3498db' },
                    { label: '단백질', value: menu.nutrition.protein, color: '#e74c3c' },
                    { label: '지방', value: menu.nutrition.fat, color: '#f39c12' },
                  ].map(n => (
                    <div key={n.label} style={{ fontSize: 10, color: n.color, fontWeight: 700 }}>{n.label} {n.value}g</div>
                  ))}
                </div>
              </div>
            ))}
          ...
          {menus.map(menu => (
            <div key={menu.id}
              ...
              <div onClick={() => setSelectedMenu(menu.id)} style={{ padding: '8px 10px 10px' }}>
                ...
                <div style={{ fontSize: 10, color: menu.kcal < 450 ? '#3B6D11' : '#e67e22', fontWeight: 600 }}>{menu.kcal}kcal</div>
        ...
        <button
          ...
          {selectedMenu && selectedTime
            ? `${menus.find(m => m.id === selectedMenu)?.name} · ${selectedTime} 예약하기 →`
            : '메뉴와 시간을 선택하세요'}
        </button>
